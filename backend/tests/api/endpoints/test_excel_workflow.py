...
from app.services.excel_parser_service import ExcelParseError, ParsedExcelData, ParsedTask
...
@pytest.fixture
def override_auth(mock_user_info):
...
class TestUploadExcelAndSimulate:
    ...
    async def test_upload_valid_excel_and_simulate_success(
        ...
        with patch("app.services.excel_parser_service.ExcelParserService.parse_excel_file") as mock_parse, \
             patch("app.services.simulation_service.SimulationService.run_simulation") as mock_simulate, \
             patch("app.services.simulation_persistence_service.SimulationPersistenceService.save_simulation_result") as mock_save:
    ...
    async def test_upload_excel_file_too_large(
        self, client: AsyncClient, mock_user_info: dict, override_auth
    ):
    ...
    async def test_upload_excel_with_parsing_errors(
        self, client: AsyncClient, valid_excel_file: io.BytesIO, mock_user_info: dict, override_auth
    ):
        ...
        with patch("app.services.excel_parser_service.ExcelParserService.parse_excel_file") as mock_parse:
            ...
            # Mock parsing error
            mock_parse.side_effect = ExcelParseError("Missing required column: task_name")
    ...
    async def test_upload_excel_with_circular_dependencies(
        ...
        with patch("app.services.excel_parser_service.ExcelParserService.parse_excel_file") as mock_parse, \
            ...
            mock_simulate.side_effect = SimulationError("Circular dependency detected: TASK-1 -> TASK-2 -> TASK-1")
    ...
    async def test_upload_excel_unauthorized(
        ...
        # Clear any auth overrides to test actual authentication requirement
        app.dependency_overrides.clear()
...
class TestDownloadSimulationExcel:
    ...
    async def test_download_simulation_excel_success(
        self, client: AsyncClient, db_session: AsyncSession, mock_user_info: dict, override_auth
    ):
        ...
        with patch(
            ...
            mock_simulation.median_duration = 51.5
            mock_simulation.std_deviation = 5.0
            mock_simulation.confidence_intervals = {10: 45.2, 50: 51.5, 90: 61.5}
            mock_simulation.iterations = 10000
            mock_simulation.task_count = 10
            mock_simulation.created_at = datetime.now()
            mock_get.return_value = mock_simulation

            # Mock Excel generation (final step - save to bytes)
            excel_bytes = b"fake excel content"
            mock_save_bytes.return_value = excel_bytes

            response = await client.get(f"/api/v1/excel/simulations/{simulation_id}/excel")
...
class TestExcelWorkflowIntegration:
    ...
    async def test_database_persistence(
        ...
        with patch("app.services.excel_parser_service.ExcelParserService.parse_excel_file") as mock_parse, \
            ...
            mock_simulation_result.confidence_intervals = {10: 4.0, 50: 5.0, 90: 6.0}
            mock_simulation_result.mean_duration = 5.0
            mock_simulation_result.median_duration = 5.0
            mock_simulation_result.iterations_run = 1000
            mock_simulation_result.task_count = 1
            mock_simulate.return_value = mock_simulation_result
